...
- **P2 is now three blocks only**: the greeting, the serif role line, and the promise line. The career sentence ("For most of my career…") and "Sometimes that means setting the direction…" were **cut by the owner on 25 Aug**. Do not reinstate them.
- **The three role lines** ("Fractional COO · Operating Partner" / "Programme & Delivery Director" / "Fractional COO") appear in two places that must stay in step: the homepage doorway cards and the "Who I help" nav (desktop dropdown + mobile sheet). Both use the `eyebrow` treatment in **deep accent `#7E5E27`**, not `--color-gold`: at 11px Aged Gold reads 2.04:1 on the clay wash and the `#9A7735` deep gold 3.13:1, both under AA. ⚠ Known dip: on the dropdown's **gold hover fill** the role line falls to 3.90:1. Fixes if it ever matters are to darken the line to `#6B4F1C` (4.97 everywhere) or drop the hover fill from 38% to 18%; both were measured and left alone rather than changing an existing interaction.
