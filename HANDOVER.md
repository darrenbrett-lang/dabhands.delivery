...
**Disclosure** (progressive "expand"): no button/pill chrome. A generic **`Expand`** label renders as a **`+` toggle** (accent-coloured, rotates to `×` on open); a descriptive label (e.g. "Explore the challenge", "See what changes") renders as **coloured bold text**. Accent deepened to a legible shade (`ACCENT[accent].trigger`: lavender→`#6E5A86`, peach→`#9E5B3A`, sage→`#5E6B3F`). Collapsed by default; reveals a left-ruled body.
...
- **Darren portrait — placed; two follow-ups.** `darren_new-image.png` (transparent cut-out) is anchored bottom-right over the clouds on `lg`+. (1) **Heavy asset:** it's a ~2.3 MB PNG — convert to WebP (with alpha) to cut the homepage payload. (2) **Mobile/tablet** (`<lg`) shows copy-only (no portrait) — decide whether to add a stacked portrait treatment there. The old `darren_jesus.jpeg` (halo) is unused.
