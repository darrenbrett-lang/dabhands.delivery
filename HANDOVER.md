...
**Disclosure** (progressive "expand"): **every trigger is a `+` toggle** (accent-coloured, rotates to `×` on open) — no descriptive text or button/pill chrome. Descriptive labels were dropped (they read as noise); each disclosure's `label` is kept only as the accessible name. Accent deepened to a legible shade (`ACCENT[accent].trigger`: lavender→`#6E5A86`, peach→`#9E5B3A`, sage→`#5E6B3F`). Collapsed by default; reveals a left-ruled body.
