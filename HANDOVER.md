...
**Disclosure** (progressive "expand"): no button/pill chrome. A generic **`Expand`** label renders as a **`+` toggle** (accent-coloured, rotates to `×` on open); a descriptive label renders as **bold, accent-coloured text offset slightly right + below** (`md:ml-8`, `font-bold`) so placement/colour/weight signal the link without verb noise — use a topic label ("The challenge"), not a verb instruction ("Explore the…"). Accent deepened to a legible shade (`ACCENT[accent].trigger`: lavender→`#6E5A86`, peach→`#9E5B3A`, sage→`#5E6B3F`). Collapsed by default; reveals a left-ruled body.
