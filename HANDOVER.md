...
- **Working branch: `staging`** (pushed to `origin/staging`; see `git log` for the latest hash): operator-template rooms, the charcoal point-of-view section, soft turnstile cards, the hover `PathwayPicker`, and per-page testimonial timing. The whole v2.0 + mastery build lives here.
...
- **One journey, not many marks.** The trajectory is the **core visual language**: a single, slightly imperfect, confident line (a racing line / conductor's gesture) built as `Trajectory.tsx`. It enters the hero and passes *behind* the headline, drawing on at load so it reads as already-in-motion (momentum, not speed). Momentum without clutter. (An earlier turnstile reprise dividing into three doors was later removed to keep that section calm.) (A signature mark beside the wordmark was tried, then removed at the owner's call — the logo is just the wordmark in the serif.) Reduce motifs ~60%; make the few that remain work harder. Typography does most of the work.
...
3. **Point of view** (charcoal, bone copy) — **two columns** (`lg:grid-cols-[1.15fr_1fr]`, `lg:items-center`), **no image** (typography carries it; the fold was removed). **Left** = the commanding thesis "The tools are changing. The problems aren't." (serif, up to `xl:88px`, tight leading/tracking, fills the column). **Right** = the argument that builds: "Most organisations already have what they need." → ingredients "Strategy. Creative ambition. Investment. Good people. Capable partners." → the insight "The challenge isn't creating more. It's helping what already exists **move together**." (plain body) → the stakes "Because somewhere between ambition and execution, work gets diluted. Momentum slips. Complexity takes hold." → the resolution "It deserves to arrive as intended." (serif, `HandUnderline` sage on "as intended"). Columns stack on mobile (thesis then argument). No eyebrow; the thesis leads.
4. **Proof** (paper) — "Trusted with important work by" + `<LogoTicker>` (**all 13 clients**, seamless marquee). No testimonial here — testimonials live only on the doorway pages.
5. **Turnstile** (bone) — "Where do you need help keeping important work moving?" + three cards, **each a defined box** (neutral `bg-paper` + `border-stone` + soft shadow at rest; on hover the card takes a soft **block of its section colour** with an accent border + accent arrow, matching the nav dropdown and the P1 overlay), title, diagnosis, support, and a clear **"Explore →"** CTA. Each links to its room. (The earlier desktop trajectory reprise above the cards has been removed.)
...
| **House accent — Deep Moss** (Trajectory, default underline, homepage brand) | `moss` | `#5B6A58` |
| Audience — Business & agency (organisational flow) | `sage` | `#A8B5A2` |
| Audience — Marketing (creative impact) | `peach` | `#E6B39A` |
| Audience — Creators & founders (growth / possibility) | `lavender` | `#B8A2D8` |
...
Ratio ~70% bone/paper, ~20% ink/graphite/stone, ~7% moss/sage, ~2% lavender, ~1% peach — per **Colour System v3** (operating-partner update; colour is meaning, not decoration). **Deep Moss `#5B6A58` is the house accent** (the `Trajectory` mark, default `HandUnderline`, homepage) — control entering the system, no longer an audience colour. **Each audience room takes one accent as a navigational cue** (Business **sage**, Marketing **peach**, Creators **lavender**); the **hero wash** is that accent fading top-to-bottom (`ACCENT[accent].wash`). **Testimonials are now neutral evidence cards (warm paper, light border, a thin 3px accent rule on top, charcoal text) — off the plum; Plum survives only on the homepage final CTA.** Same type/spacing/layout/core palette otherwise. Tokens in `globals.css @theme`. Utilities: `.eyebrow` (Manrope uppercase label), `.font-serif`, `.signature-gradient`. (`.halo-glow` removed — halo motif retired.) Focus ring is coral. Legacy `dab-*` tokens remain defined but nothing live uses them (prune, see TODO).
