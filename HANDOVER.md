...
- **Darren portrait**: `/images/darren_jesus.jpeg` (owner-supplied, landscape 3:2; white sunglasses, neon halo, pink sky). Reintroduces the halo, counter to the earlier no-halo portrait direction, kept at owner's request. Single crop wired (mobile = desktop); add a portrait/mobile crop via `Figure`'s `desktop` prop for art-directed mobile framing.
