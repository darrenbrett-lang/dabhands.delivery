...
5. **Turnstile** (bone) — "Where do you need help keeping important work moving?" + three cards, **each a soft indent of its room accent** (a whisper of moss / peach / sage over bone, neutral `border-stone` + soft shadow that lifts and brings the accent border forward on hover), title, diagnosis, support, and a clear **"Explore →"** CTA in the accent. Each links to its room. (The earlier desktop trajectory reprise above the cards has been removed.)
