...
5. **Turnstile** (bone) — "Where do you need help keeping important work moving?" + three cards, **each a defined charcoal-on-bone box** (`bg-paper` lifted off the bone section by a `border-stone` hairline + a soft layered shadow that deepens on hover; the accent tints were dropped for clear box contrast), title, diagnosis, support, and a clear charcoal **"Explore →"** CTA. Each links to its room. (The earlier desktop trajectory reprise above the cards has been removed.)
