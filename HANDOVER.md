...
- **Darren portrait (in progress).** The Darren module is now a **full-bleed `clouds.png` banner** with the copy on the left and the **right reserved for Darren's cut-out portrait**, which the owner is supplying separately. When it lands, composite it over the clouds on the right (likely an absolutely-positioned PNG cut-out anchored bottom-right). The old `darren_jesus.jpeg` (halo) is no longer referenced.
