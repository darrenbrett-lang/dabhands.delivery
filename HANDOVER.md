...
1. **Hero** (bone, centred) — Instrument Serif "Keeping important work moving." (per-word stagger on mount) + Manrope subline. The **`Trajectory`** enters from the left and passes *behind* the headline, drawing on at load (moss, low opacity, fades at both ends). Section is `relative overflow-hidden`; content sits above the line at `z-10`. Below the subline, a quiet **"Who I help →" cue** (`PathwayPicker`) opens a small dialog that lays the three pathways out as a three-choice navigation (Escape and backdrop close, focus moves into the panel then returns to the trigger, scroll locked, reduced-motion aware): an optional shortcut straight into a doorway for a decisive visitor.
...
3. **Point of view** (bone) — **two columns** (`lg:grid-cols-[1.15fr_1fr]`, `lg:items-center`), **no image** (typography carries it; the fold was removed). **Left** = the commanding thesis "The tools are changing. The problems aren't." (serif, up to `xl:88px`, tight leading/tracking, fills the column). **Right** = the argument that builds: "Most organisations already have what they need." → ingredients "Strategy. Creative ambition. Investment. Capability. Good people. Trusted partners." → the insight "The challenge isn't creating more. It's helping what already exists **move together**." (serif, `HandUnderline` moss on "move together") → the stakes "Because somewhere between idea and market, work gets diluted. Momentum slips. Complexity takes hold." → the resolution "It deserves to arrive as intended." (serif). Columns stack on mobile (thesis then argument). No eyebrow; the thesis leads.
...
5. **Turnstile** (bone) — "Where do you need help keeping important work moving?" + three cards, **each tinted in its room accent** (moss / peach / sage block colour, with a "That's me →" cue + accent border deepening on hover), title, diagnosis, support. Each links to its room. **Desktop adds a trajectory reprise above the cards**: one line divides into three faint branches in the room colours, each landing over its column (`hidden md:block`).
...
- **`PathwayPicker.tsx`** — the homepage hero quick-nav. A trigger button ("Who I help →") opens an accessible `role="dialog"` overlay (framer `AnimatePresence`) with the three pathways as accent-tinted tiles (moss / peach / sage) linking into each room. Escape + backdrop close, body scroll lock, focus into the panel and back to the trigger on close, reduced-motion aware.
