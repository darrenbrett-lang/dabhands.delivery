...
Everything below is **merged to `main` and deployed**. Production is `https://dabhands.delivery` via Vercel (every push to `main` auto-deploys). Most recent prod commit: `d3779f3` "Crown mark redesign, Pause for Thought rewrite, four-step strategy flow, 0.8 headline leading". `main` = `origin/main`, working tree clean (HANDOVER edit pending commit).
...
**First private page: `/for/manifesto-digital`** ("Manifesto Digital: Building a modern delivery operating system" — pitch for Rebecca Hull). Section flow (bg rhythm): sand hero (byline lockup: 44px circular headshot `darren-brett_colour_headshot.jpeg` + eyebrow "Prepared by Darren Brett for Rebecca Hull, Manifesto Digital" → kicker "Manifesto Digital" → H1 → subtitle "A perspective on the journey to 2027" → intro "How Manifesto can strengthen delivery excellence today…") → **white manifesto statement** ("Every operational and delivery decision should reinforce our strategy…", centred, with a four-lozenge flow: MANIFESTO STRATEGY → CLIENT IMPERATIVES → FUTURE CONSUMER & TECHNOLOGY SHIFTS → DELIVERY OPERATING SYSTEM as charcoal pills with green text, horizontal at xl, vertical ↓ cascade below) → **white "The forces in play"** (intro line + 13-item `ChipExplorer`) → **sand thesis** ("The role of Delivery Operations… To translate strategy into execution…") → **charcoal 3×3 framework matrix** (see below) → **sand "Manifesto 2027" destination** (crown mark + statement) → **white "Characteristics of a high-performing system"** (10-item `ChipExplorer`, "What it feels like when it's working.") → **charcoal Outcomes** (6 cards with green icons; sixth is "People & Culture") → **white "Beneath the System"** (two-systems comparison: visible system = bordered white card / invisible system = charcoal card with green eyebrow; closing statement with `HandUnderline tone="light"` on "together") → **charcoal "A Pause for Thought"** (the hero `Ribbon` returns at 0.16 opacity; manifesto-stream copy in a 42ch centred column alternating quiet cream/80 lines with bold full-cream beats; asks why people will visit client digital destinations as AI reshapes discovery; ends on the page's one green type moment: "If AI changes why people come, how should we change what we build?") → **sand closing** ("The dots are already here. Perhaps it's simply a case of connecting them." + "Compare notes" CTA → mailto subject "Comparing Notes" with pre-filled body "Hi Darren, I've been reading your perspective…").

**Typography on this page:** all big statement headlines run `leading-[0.8]` (deliberately tight; verified no glyph collisions). Exceptions: hero H1 at 1.02, the green Pause finale at 1.12. Eyebrows at `/75` opacity. **The whole page passes WCAG AA** (audited programmatically; footer + matrix axis label were bumped to `/60` cream to clear 4.5:1).
...
**Crown mark redesigned (SITE-WIDE).** Feedback said the old crown read as a mountain range. The shared SVG now has a closed solid band, steeper sides and jewels seated on all three points (same hand-drawn stroke language + SMIL dot flash). It also now renders at **half its old size** everywhere (`w-[60px]` mobile → `lg:w-[100px]`) so it reads as an editorial mark, not a hero graphic. All five `<img>` references are cache-busted to `?v=2` because `/images/*` ships immutable cache headers — keep bumping the version on any future edit to that file.
...
Multi-page site (Home / When To Bring Us In / Experience / Contact) plus **unlisted `/for/<company>` pitch pages** (currently `/for/manifesto-digital`). On production at `https://dabhands.delivery` via Vercel's GitHub integration — every push to `main` auto-deploys. `main` = `origin/main` = `d3779f3`.
...
- **Working branch**: `main`, in sync with `origin/main` at `d3779f3` (apart from this HANDOVER edit).
...
- **Crown mark** (`/public/images/dab-hands-crown-mark.svg`) — line-art crown, **redesigned this session** (closed solid band, steeper sides, jewels seated on the three points — the old open-ridgeline drawing misread as mountains). Used on home Core Truth, home closing, Experience closing, where-we-step-in closing, and the `/for/manifesto-digital` "Manifesto 2027" section — all at **half size** (`w-[60px]`→`lg:w-[100px]`) and referenced as `?v=2` (immutable cache on `/images/*`; bump the version on any edit). The 3 jewels have **SMIL `<animate>` flash** on opacity 1 → 0.2 → 1 over 1.8s, in sync. Colour inherits `currentColor`.
