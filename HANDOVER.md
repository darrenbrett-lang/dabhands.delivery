...
3. **Point of view** (bone) — a **centred manifesto / crescendo** (`text-center`, `max-w-[880px]`, `py-24 md:py-36`), no columns: provocation "The tools are changing. The problems aren't." (serif) → "Most organisations already have what they need." → ingredients line "Strategy. Creative ambition. Investment. Good people. Trusted partners." → the **`fold.png` mark, centred (`max-w-[300px] md:[340px]`), as a single form under tension — the pivot** → the insight "The challenge isn't creating more. It's helping what already exists **move together**." (serif, `HandUnderline` moss on "move together") → the stakes "Because somewhere between idea and market, momentum slips and value gets lost." → the resolution "It deserves to arrive as intended." (serif). Serif sizes step **down** thesis → insight → resolution (momentum settling). No eyebrow; the thesis leads.
