...
> **Rebuild in progress — operator template.** Per the Destination Page brief, the rooms are moving to a new 8-section **operator** template (`components/OperatorTemplate.tsx`): DAB Hands as a trusted **operating partner, not a consultant**; **"I" voice**, recognition over explanation, **situations not services**. Spine: Hero (recognition headline + simple subline + "20 years inside [brands]" trust badge + **charcoal CTA**, all above the fold) → Validation ("you've done the hard part") → Diagnosis (**4 cards, 4th = the gap nobody owns**) → Outcomes (behavioural) → Transition ("That's where I step in") → Where I tend to help (**3 situation cards**) → Trust (plum quote) → Close. Mobile **sticky CTA**; per-room accent; content lives in the page file. **Business & agency leaders and Marketing leaders are on it now** (content in their page files; the diagnosis section accepts either 4 cards or prose `paras`); **Creators & founders is still on the legacy `AudienceTemplate` below — replicate next.**

#### Legacy template — `components/AudienceTemplate.tsx` (Creators only)
